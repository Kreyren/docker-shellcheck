...
case "$SPEEDTEST_TRIES" in
	...
	disabled) die 1 "Angry Kreyren: WHO DARED TO REMOVE MY SPEEDTEST_TRIES TRAP?! Those are here for a reason u know! (x_X)" ;;
...
case "${DISTRO##ID\=}" in
	debian)
		if ! command -v apt >/dev/null; then
			die 1 "This debian does not have expected apt, runtime is not adapted to handle this situation"
		elif command -v apt >/dev/null; then
			# Make sure that we have all required dependencies
			$SUDO apt install -y curl netselect-apt bc || die 1 "Script '$myName' was unable to install all required dependencies for it's runtime"
		else
			die 255 "checking apt executability in $myName"
		fi
	;;
